...
{
	// Порог "похожести" двух предложений (от 0 до 1).
	// Если пересечение слов у "удалённого" и "добавленного" предложения
	// выше этого числа — считаем, что это одно изменённое предложение,
	// а не два разных (одно исчезло, другое появилось само по себе).
	 ...
	 *         Массив "сегментов" для фронта: каждый — это одно предложение
	 *         с типом (unchanged/added/deleted/modified) и текстом.
	 */
	public function compare(string $oldText, string $newText): array
	{
		// Шаг 1: текст -> массив предложений
		$oldSentences = $this->splitIntoSentences($oldText);
		$newSentences = $this->splitIntoSentences($newText);

		// Шаг 2: сравниваем два массива предложений, получаем "сырую"
		// последовательность операций: какие предложения совпали (equal),
		// какие пропали (delete), какие появились (insert)
		$ops = $this->diff($oldSentences, $newSentences);

		// Шаг 3: среди пропавших/появившихся ищем похожие пары
		// и помечаем их как "изменено", а не "удалено + добавлено"
	 ...
	 *
	 * Пример: "Кот сидел. Он спал." -> ["Кот сидел.", "Он спал."]
	 *
	 * @return string[]
	 */
	private function splitIntoSentences(string $text): array
	{
		...
		if($text === '')
		{
			// Пустой текст — пустой массив предложений, а не ошибка
			return [];
		}

		// Схлопываем несколько пробелов/переносов строк в один пробел,
		// чтобы "Кот   сидел.\n\nОн спал." не ломало разбиение
		$text = preg_replace('/\s+/u', ' ', $text) ?? $text;

		// Регулярка: "любые символы, затем один или несколько знаков
		// конца предложения (.!?…), после которых идёт пробел или конец строки"
		// Так мы режем текст на куски, каждый из которых заканчивается
		// знаком препинания (либо это последний кусок без знака в конце)
		...
		foreach($matches[0] as $chunk)
		{
			...
			if($chunk !== '')
			{
				// Отбрасываем пустые куски, которые иногда даёт регулярка
				$sentences[] = $chunk;
			}
		}
	...
	private function diff(array $a, array $b): array
	{
		...
		// Таблица динамического программирования размером (m+1) x (n+1).
		// L[i][j] = длина самой длинной общей подпоследовательности
		// для "хвостов" массивов a (начиная с i) и b (начиная с j).
		// Строим её с конца массивов к началу.
	...
	private function pairGap(array $gap): array
	{
		...
		// Жадно разбираем пары: берём самую похожую, "бронируем" оба
